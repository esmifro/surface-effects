...
Inductive TcHeap : (Heap * Sigma * Rho) -> Prop := 
  | TC_Heap : forall heap store rho,
                (forall k v,
                ...
                (forall k v t,
                   (find_H k heap = Some v ->
                    find_ST k store = Some t ->
                    TcVal (store, v, subst_rho rho t))) ->
               TcHeap (heap, store, rho).
...
Lemma same_domain:
  forall stty heap rho, 
    TcHeap (heap, stty, rho) ->
    (forall k v t,
       find_ST k stty = None -> 
...
Lemma update_heap_fresh:
  forall stty heap rho,
    ...
    (forall k v t,
       ...
       TcHeap (update_H (k, v) heap, update_ST (k, t) stty, rho)).
Proof. 
  intros stty heap rho Hhp l v t Htc STfind.
...
Lemma update_heap_exists:
  forall stty heap rho,
    ...
    (forall k v t,
       find_ST k stty = Some t ->
       TcVal (stty, v, subst_rho rho t) ->
       TcHeap (update_H (k, v) heap, stty, rho)).
Proof. 
  intros stty heap rho Hhp l v t Htc STfind.
  inversion Hhp as [? ? ? TcHeap_STFind TcHeap_HFind TcHeap_tcVal]; subst; constructor.
